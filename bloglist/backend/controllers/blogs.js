...
blogsRouter.post('/', async (request, response, next) => {
  ...
  try {
    ...
    if (!user || !user.id) {
      // userExtractor asigna null si no hay token o es inválido
...
blogsRouter.delete('/:id', async (request, response, next) => {
  ...
  try {
    ...
    if (user) {
      // Este 'if' es una doble seguridad, request.user ya debería ser un objeto aquí
      user.blogs = user.blogs.filter(
        (blogRef) => blogRef.toString() !== blogId.toString(),
      )
      await user.save()
    }
...
// --- Ruta POST /api/blogs/:id/comments - Agregar un comentario a un blog ---
